...
const base_url = window.location.origin + '/hugo-test';
...
function executeSearch() {
	...
	fetchJSONFile(base_url + '/index.json', function(data){
		...
		for (let i in results.slice(0, n)) {
		    ...
		    searchItems = searchItems + '<li><a href="' + result.permalink + '" tabindex="0">' + '<span class="title">' + result.title + '</span><br /> <span class="sc">'+ result.tags +'</span> — ' + result.date + ' — <em>' + result.contents.slice(0, 20) + '</em></a></li>';
		}
